...
#define USE_ENERGY_OPTIMIZATION 0 
...
class ImageSeamCarver
{
public:
	ImageSeamCarver(std::string filename, int numOfSeams);
	~ImageSeamCarver();
	...
	void LoadImage(std::string filename, int& outWidth, int& outHeight, int& outChannelNum);
...
private:
	...
	std::vector<Pixel*> GetPixelNeighbors(const int indexX, const int indexY);

	void RemovePixelColumn();
